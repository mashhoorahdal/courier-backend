...
from .models import Order, DeliveryBoy, Delivery
...
def admin_dashboard(request):
    ...
    # Get recent orders
    recent_orders = Order.objects.select_related("customer").order_by("-created_at")[
        :10
    ]
...
def admin_users_create(request):
    ...
    return render(
        request, "admin/user_form.html", {"form": user_form, "title": "Create New User"}
    )
...
def admin_delivery_boys_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"Delivery boy {delivery_boy.user.username} created successfully!",
            )
    ...
    return render(
        request,
        "admin/delivery_boy_form.html",
        {"form": form, "title": "Create New Delivery Boy"},
    )
...
def admin_delivery_boys_edit(request, delivery_boy_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"Delivery boy {delivery_boy.user.username} updated successfully!",
            )
            return redirect("admin_delivery_boys")
    else:
...
def admin_delivery_boys_delete(request, delivery_boy_id):
    ...
    return render(
        request,
        "admin/delivery_boy_confirm_delete.html",
        {"delivery_boy": delivery_boy},
    )
...
def admin_assign_delivery(request, order_id):
    ...
    if request.method == "POST":
        ...
        Delivery.objects.create(
            order=order, delivery_boy=delivery_boy, status="assigned"
        )
        ...
        messages.success(
            request,
            f"Order {order.barcode} assigned to {delivery_boy.user.get_full_name() or delivery_boy.user.username}.",
        )
...
def admin_update_delivery_status(request, delivery_id):
    ...
    if request.method == "POST":
        ...
        if new_status == "picked_up" and delivery.status == "assigned":
            ...
            delivery.save()
            messages.success(
                request, f"Delivery {delivery.order.barcode} marked as picked up."
            )
            ...
            messages.success(
                request, f"Delivery {delivery.order.barcode} marked as delivered."
            )
            ...
            delivery.save()
            messages.warning(
                request, f"Delivery {delivery.order.barcode} marked as failed."
            )
